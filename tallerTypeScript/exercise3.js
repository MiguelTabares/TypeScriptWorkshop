...
function isPalindrome(str) {
    // a. Convert the string to lowercase and remove whitespace.
    var formatString = str.toLowerCase().replace(/\s/g, '');
    // b.1. Obtain the inverted chain
    var invertedString = formatString.split('').reverse().join('');
    // b.2. Compare the original string with the inverted string.
    var palindrome = formatString === invertedString;
    // c. Display a message indicating whether the string is a palindrome or not.
    if (palindrome) {
        console.log("\"".concat(str, "\" IS A PALINDROME."));
    }
    else {
        console.log("\"".concat(str, "\" IS NOT A PALINDROME."));
    }
    return palindrome;
}
